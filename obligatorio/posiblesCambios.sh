...
######################## Buscar un find que no me liste los directorios ##############
        ...
        if [ $i != "-m" ] && [ $i != "-o" ] && [ $i != "-r" ]; then
            ...
            if [ -d $i ]; then
                ...
                if [ -r $ruta ] && [ -x $ruta ]; then
                    ...
                    ## En el cambio esto podria evaluarse dentro del [-d parametro final] y dentro poner if [-d $1]
                        lista-SinFiltros-CONUbicacion $i
                        echo ''
                        contar_archivos $i
                        echo ''
                        echo "Opcion 7 - SIN FILTRO SOLO /UBI"
                        echo "Parametro1"$1,"Parametro2"$2,"Parametro3"$3
                        echo '-------------------------------'
                    fi
...
##################################################################################################################################
#                                     Evaluacion sin ubicacion como parametro                                                    #
##################################################################################################################################
...
### si esto fuera un elif puedo empezar a poner aca las condiciones sin ubicacion y por else le tiro el mensaje de error,
### ademas, puedo seguir usando la misma estructura creada por nosotros y borrar lo que le copiamos a Edgardo.
# Fuente: https://linuxize.com/post/how-to-compare-strings-in-bash/
